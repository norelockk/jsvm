...
let ltree = new Tree();  /* dynamic length/symbol tree */
let dtree = new Tree();  /* dynamic distance tree */

function Data(_source, _dest) {
}
...
function tinf_read_bits(num, base) {
  if (!num)
    return base;

  while (bitcount < 24) {
    $tag |= $source[$sourceIndex++] << bitcount;
    bitcount += 8;
  }
...
function tinf_decode_trees(lt, dt) {
...
function tinf_inflate_block_data(lt, dt) {
  while (1) {
...
function tinf_inflate_uncompressed_block() {
  ...
  /* get length */
  length = $source[$sourceIndex + 1];
  length = 256 * length + $source[$sourceIndex];

  /* get one's complement of length */
  invlength = $source[$sourceIndex + 3];
  invlength = 256 * invlength + $source[$sourceIndex + 2];
  ...
  /* copy block */
  for (i = length; i; --i)
    dest[destLen++] = $source[$sourceIndex++];
...
function tinf_uncompress(source, dest) {
  ...
  do {
    ...
    switch (btype) {
      ...
      case 1:
        ...
        res = tinf_inflate_block_data(sltree, sdtree);
        break;
      case 2:
        /* decompress block with dynamic huffman trees */
        tinf_decode_trees(ltree, dtree);
        res = tinf_inflate_block_data(ltree, dtree);
        break;
      default:
        res = TINF_DATA_ERROR;
    }
  ...
  if (destLen < dest.length) {
    if (typeof dest.slice === 'function')
      return dest.slice(0, destLen);
    else
